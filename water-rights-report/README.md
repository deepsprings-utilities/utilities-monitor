...
- Optional: **`WATER_RIGHTS_SERIAL`** or **`WATER_RIGHTS_DEVICE_ADDRESS`** (e.g. `mb-006`); **`REPORT_YEAR`**, **`REPORT_END`**, **`REPORT_TZ`**, and static columns (see `generate-a1-report.mjs`). The generator fails instead of writing empty artifacts when no flow rows match; set **`ALLOW_EMPTY_REPORT=1`** only for an intentional blank report.
