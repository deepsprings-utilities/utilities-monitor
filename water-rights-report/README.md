...
- **`WATER_RIGHTS_FLOW_SCALE`** — multiply stored GPM before the workbook. Default **4** for Wyman (calibration scale), **1** for booster (raw GPM). Both streams use the same env name; CI sets per-job defaults via **`WATER_RIGHTS_FLOW_SCALE`** / **`BOOSTER_FLOW_SCALE`** Variables.
