...
import { ingestBatchLimit, selectObjectsForProcessing } from "./ingest-selection.js";
...
async function main() {
  ...
  const maxKeys = ingestBatchLimit(process.env.INGEST_BATCH_LIMIT || "200");
  ...
  const selection = selectObjectsForProcessing(listedObjects, processedSet, maxKeys);
  const objects = selection.objectsToProcess;
  const stats = {
    listed: listedObjects.length,
    skipped: selection.skippedProcessed,
    deferred: selection.deferredPending,
    succeeded: 0,
    failed: 0,
  };
  console.log(
    `run_id=${runId} prefix=${prefix} max_objects_this_run=${maxKeys} list_scan_cap_env=${process.env.INGEST_LIST_SCAN_CAP || "default"} listed=${listedObjects.length} selected=${objects.length} checkpoint_skipped=${selection.skippedProcessed} deferred_pending=${selection.deferredPending} dry_run=${dryRun}`,
  );
  ...
  await db.end();
  console.log(
    `run_complete run_id=${runId} listed=${stats.listed} skipped=${stats.skipped} deferred=${stats.deferred} succeeded=${stats.succeeded} failed=${stats.failed}`,
  );
  if (
    stats.listed > 0 &&
