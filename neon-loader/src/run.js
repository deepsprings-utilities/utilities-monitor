...
async function main() {
  ...
  const selection = selectObjectsForIngest(candidateObjects, processedSet, maxKeys);
  const objects = selection.objects;
  const stats = {
    listed: selection.candidates,
    skipped: selection.alreadyProcessed,
    succeeded: 0,
    failed: 0,
  };
  console.log(
    `run_id=${runId} prefix=${prefix} max_objects_this_run=${selection.processLimit} list_scan_cap_env=${process.env.INGEST_LIST_SCAN_CAP || "default"} listed=${selection.candidates} already_processed=${selection.alreadyProcessed} selected=${objects.length} dry_run=${dryRun}`,
  );
  ...
  await db.end();
  console.log(
    `run_complete run_id=${runId} listed=${stats.listed} skipped=${stats.skipped} selected=${objects.length} succeeded=${stats.succeeded} failed=${stats.failed}`,
  );
  if (
    stats.listed > 0 &&
