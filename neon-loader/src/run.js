...
async function main() {
  ...
  const selection = await selectUnprocessedObjects(db, listedObjects, maxKeys);
  const objects = selection.objects;
  const stats = {
    scanned: listedObjects.length,
    checkpointed: selection.alreadyProcessed,
    selected: objects.length,
    succeeded: 0,
    failed: 0,
  };
  console.log(
    `run_id=${runId} prefix=${prefix} max_objects_this_run=${maxKeys} list_scan_cap_env=${process.env.INGEST_LIST_SCAN_CAP || "default"} scanned=${listedObjects.length} checkpointed=${selection.alreadyProcessed} selected=${objects.length} dry_run=${dryRun}`,
  );
  ...
  await db.end();
  console.log(
    `run_complete run_id=${runId} scanned=${stats.scanned} checkpointed=${stats.checkpointed} selected=${stats.selected} succeeded=${stats.succeeded} failed=${stats.failed}`,
  );
  if (
    stats.scanned > 0 &&
    stats.checkpointed === stats.scanned &&
    stats.succeeded === 0 &&
    stats.failed === 0
  ) {
    console.warn(
      `warning run_complete all_scanned_objects_checkpointed scanned=${stats.scanned} (every scanned key already in ingest_checkpoint; no new inserts this run)`,
    );
  }
}
