...
import { processLimitFromValue, selectObjectsForIngest } from "./ingest-selection.js";
...
async function main() {
  ...
  const maxKeys = processLimitFromValue(process.env.INGEST_BATCH_LIMIT || "200");
  ...
  const { selected: objects, skippedAlreadyProcessed } = selectObjectsForIngest(
    listedObjects,
    processedSet,
    maxKeys,
  );
  ...
  console.log(
    `run_id=${runId} prefix=${prefix} max_objects_this_run=${maxKeys} list_scan_cap_env=${process.env.INGEST_LIST_SCAN_CAP || "default"} listed=${listedObjects.length} selected=${objects.length} skipped_checkpointed=${skippedAlreadyProcessed} dry_run=${dryRun}`,
  );
  ...
  await db.end();
  console.log(
    `run_complete run_id=${runId} listed=${stats.listed} selected=${stats.selected} skipped=${stats.skipped} succeeded=${stats.succeeded} failed=${stats.failed}`,
  );
  if (
    stats.listed > 0 &&
    stats.selected === 0 &&
