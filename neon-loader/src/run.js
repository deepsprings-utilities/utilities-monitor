...
import { countProcessedObjects, normalizeEtag, selectObjectsForIngest } from "./ingest-selection.js";
...
async function main() {
  ...
  const alreadyProcessed = countProcessedObjects(scannedObjects, processedSet);
  const objects = selectObjectsForIngest(scannedObjects, processedSet, maxKeys);
  ...
  console.log(
    `run_id=${runId} prefix=${prefix} max_objects_this_run=${maxKeys} list_scan_cap_env=${process.env.INGEST_LIST_SCAN_CAP || "default"} listed=${scannedObjects.length} already_processed=${alreadyProcessed} selected=${objects.length} dry_run=${dryRun}`,
  );

  for (const object of objects) {
    const etag = normalizeEtag(object.etag);
