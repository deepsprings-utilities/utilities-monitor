...
async function main() {
  ...
  stats.skipped = selection.skippedProcessed;
  stats.deferred = selection.deferred;

  console.log(
    `run_id=${runId} prefix=${prefix} max_objects_this_run=${maxKeys} list_scan_cap_env=${process.env.INGEST_LIST_SCAN_CAP || "default"} listed=${listedObjects.length} selected=${objects.length} skipped_checkpointed=${stats.skipped} deferred_unprocessed=${stats.deferred} dry_run=${dryRun}`,
  );
  ...
  await db.end();
  console.log(
    `run_complete run_id=${runId} listed=${stats.listed} skipped=${stats.skipped} succeeded=${stats.succeeded} failed=${stats.failed} deferred_unprocessed=${stats.deferred}`,
  );
  if (
    stats.listed > 0 &&
