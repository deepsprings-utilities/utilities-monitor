...
/**
 * Lists objects under prefix, prioritizing **newest LastModified first**. R2/S3 returns keys in
 * **lexicographic key order**; without sorting, each run could see the same first N keys and
 * never reach newer uploads. The ingest runner applies `maxKeys` after checkpoint filtering so
 * older unprocessed backlog is not starved by newer already-processed objects.
 *
 * @param {object} opts
 * @param {number} [opts.maxKeys] — process-per-run limit; used as the minimum scan cap
