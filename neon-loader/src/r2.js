...
/**
 * Lists objects under prefix, prioritizing **newest LastModified first**.
 * R2/S3 returns keys in **lexicographic key order**; callers must checkpoint-filter the scanned
 * window before applying their per-run process limit, or an older backlog can be starved forever.
 *
 * @param {object} opts
 * @param {number} [opts.maxKeys] — process-per-run limit; used as a minimum scan size; default 200
