 ...
 * returns keys in **lexicographic key order**; without this, each run could
 * see the same first N keys (all already checkpointed) and never reach newer uploads.
 * The caller applies the per-run processing limit after checkpoint filtering.
 *
 * @param {object} opts
 * @param {number} [opts.maxKeys] — per-run process cap, used as a minimum scan cap; default 200
