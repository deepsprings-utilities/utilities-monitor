...
/**
 * Lists objects under prefix, prioritizing **newest LastModified first** within the scan window.
 * R2/S3 returns keys in **lexicographic key order**; callers must filter checkpoints before
 * applying the per-run process limit so older unprocessed uploads cannot be starved by newer
 * checkpointed objects.
 *
 * @param {object} opts
 * @param {number} [opts.maxKeys] — per-run process limit; used only to keep scan cap >= limit
