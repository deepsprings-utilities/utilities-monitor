...
/**
 * Lists objects under prefix, prioritizing **newest LastModified first**. R2/S3 returns
 * keys in **lexicographic key order**; callers must apply the per-run batch limit only
 * after checkpoint filtering so older unprocessed uploads are not starved by newer
 * already-processed keys.
 *
 * @param {object} opts
 * @param {number} [opts.maxKeys] — process-per-run limit used as the minimum scan cap; default 200
