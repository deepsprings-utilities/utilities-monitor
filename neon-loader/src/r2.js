...
/**
 * Lists objects under prefix, prioritizing **newest LastModified first**. R2/S3 returns keys
 * in **lexicographic key order**; callers need the full scan window so checkpoint filtering can
 * happen before the per-run processing limit is applied.
 *
 * @param {object} opts
 * @param {number} [opts.maxKeys] — process-per-run limit; used as a floor for scan cap; default 200
