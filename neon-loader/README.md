...
- Optional:
  ...
  - `INGEST_BATCH_LIMIT` (default: `200`) — **maximum uncheckpointed R2 objects to process in one ingest run**. Listing is paginated across the bucket, checkpointed keys are filtered out, then this limit is applied; raise this (e.g. `2000`, `5000`) for backlog catch-up. Each scheduled ingest run (currently **every 2 hours** via [`.github/workflows/ingest-r2-to-neon.yml`](../.github/workflows/ingest-r2-to-neon.yml)) drains up to this many unprocessed keys; backlog may need multiple runs or a larger limit (GitHub Actions jobs time out after **360 minutes** unless lowered).
  - `INGEST_LIST_SCAN_CAP` (default: `250000`) — how many keys may be scanned before checkpoint filtering and batch selection. If your prefix has more keys than this cap, ingest fails by default to avoid silent starvation.
...
Portable ops checks aligned with [Grafana alert queries](grafana/alerts/) (see `manifest.yaml`): [`.github/workflows/neon-email-alerts.yml`](../.github/workflows/neon-email-alerts.yml) runs on a schedule (hourly UTC), runs `npm run migrate` (includes `alert_notification_state` for dedupe), then `npm run notify:email-alerts`. When something fires, **the email lists only those checks** (no “OK” sections for rules that did not trigger). Supported checks: **stale data** per `physical_group` (default no row newer than **8 hours** — accounts for 2-hour ingest cadence plus GitHub Actions cron drift), **Hydro Out** (hydro kW **&lt; threshold** only if reading is recent — same idea as `hydro-out-alert.sql`), **water sampling due** (same window as `water-reporting-alert-count.sql`), **low_alarm / high_alarm** rows (lookback capped).

**Actions Secrets:** `NEON_DATABASE_URL`, `RESEND_API_KEY`, `ALERT_EMAIL_FROM` (verified sender in Resend).

**Repository Variables:** `ALERT_EMAIL_TO` (**comma-separated recipients** — stored as a Variable so you can edit without Secrets). Optional: `ALERT_STALE_AFTER_MINUTES` (default `480` = 8h; must exceed worst-case ingest lag — currently 2h cron interval + GitHub drift), `ALERT_COOLDOWN_MINUTES` (default `360`), `ALERT_HYDRO_MIN_KW`, `ALERT_HYDRO_RECENT_MINUTES`, `ALERT_ALARM_LOOKBACK_MINUTES`, `ALERT_ALARM_ROW_LIMIT`. Optional env `NOTIFY_DRY_RUN=1` skips Resend (still queries Neon). Temporary freeze switch: `DISABLE_STALE_ALERTS=1` disables stale-data emails but keeps hydro/water/alarm checks active.
